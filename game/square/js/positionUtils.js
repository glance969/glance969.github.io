// 候选位置生成与本地策略（H5 版，全局作用域）
// 算法逻辑与原版保持一致；仅做两处安全修复：
//   - getValidRemovePositions 内 debugLog 引用了未定义的 pos（原版 :410）→ 移除。
//   - 其余启发式不改动，保持对弈行为一致。

function getValidPositions(phase, currentColor, data) {
...
function getValidPlacePositions(currentColor, opponentColor, data) {
    ...
    // 3、如果己方棋子和对方棋子都不能形成阵型，则回头继续判断当己方连续放置两颗棋子时己方能否形成阵型
...
function getBestFormationPosition(row, col, tempBoard, currentColor, opponentColor, availablePositions) {
    ...
    for (let [dx, dy] of DIRECTIONS.NEIGHBORS) {
        ...
        if (tempBoard[newRow][newCol].color === currentColor) {
            hasCurrentPiece = true;
        } else if (tempBoard[newRow][newCol].color === opponentColor) {
...
function getPossibleFormationPositions(uniquePositions, tempBoard, currentColor, opponentColor) {
    ...
    const candidatePositions = dedupePositions(uniquePositions);
...
function getValidRemovePositions(currentColor, opponentColor, data) {
    ...
    let tempExtraMoves = 0;
    let tempOpponentExtraMoves = 0;
    let largestMoves = 0; // 在对方会形成阵型，移除对方阵型上的棋子后自己也能形成阵型时可获得的额外吃子数
        ...
        for (let col = 0; col < 6; col++) {
            ...
            if (isFirstRemove) {
                ...
                if (newExtraMoves > tempExtraMoves) {
                    ...
                    debugLog(CONFIG.DEBUG, `1-${currentColor}棋盘满时，先考虑自己形成阵型，该位置与之前的位置获取的奖励相等，待考虑:`, tempPosition);
                }
    ...
    // 只返回周围棋子最少的那个位置
    const leastOpponentPosition = sortedPositions[0];
